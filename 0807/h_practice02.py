...
# : menu_list = [{'떡복이': 3000}, {'어묵': 700}, {'튀김' : 500}, {'김밥': 2000}]
# : (클래스 변수 - {메뉴명: 가격} 딕셔너리 )
...
class Shop:
    ...
    def sales(cls, menu, amount):
        for item in cls.menu_list:
            if menu in item:
                # 인스턴스의 총 합계를 할당더하기 연사을 통해 구현
                # 메뉴 가격과 판매량을 곱한 후 전체 매출액에 더함
                cls.total += item[menu] * amount
                return
        # 해당 메뉴가 없다면 {menu}는 메뉴에 업습니다. 라는 f-string 반환문을 작성
        ...
        # for in 반복문
        # 메뉴 리스트에서 메뉴를 가지고 와서
        # 해당 메뉴(menu)가 메뉴 리스트에 있느지 확인
        # 인스턴스의 총 합계를 할당더하기 연산을 통해 구현
        #
        # 해당 메뉴가 없다면 {menu}는 메뉴에 없습니다. 라는 print